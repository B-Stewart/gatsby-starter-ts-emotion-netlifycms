...
export class Header extends React.Component<IHeaderProps, IHeaderState> {
  ...
  render() {
    ...
    const linkComponents = links.map(
      ({ to, name, component: Component }, i) => (
        <Link
          key={i}
          to={to}
          onClick={() => this.setMenu(false)}
          className="text-black block py-2 md:py-0 md:inline-block mr-4 uppercase cursor-pointer border-b border-neutral-300 last:border-0 md:border-b-0 hover:text-neutral-600"
        >
